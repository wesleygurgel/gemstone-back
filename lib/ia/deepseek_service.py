...
def send_prompt(prompt, model=None, temperature=0.7, max_tokens=1000, system_message=None):
    """
    Send a prompt to the Deepseek API and return the response.
    ...
    try:
        ...
        # Otherwise, return a dict that matches the structure of the previous implementation
        return {
            'choices': [
                {
                    'message': {
        ...
        error_msg = f"Error in send_prompt function: {str(e)}"
        logger.error(error_msg)
        return {"error": error_msg}
